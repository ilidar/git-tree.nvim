...
function M.show_git_log_in_buffer(buffer)
    ...
    for k, v in pairs(git_log_lines) do
        ...
        local commit_branch_from, commit_branch_to = git.get_commit_branch_range(v)
        if commit_branch_from ~= -1 then
            api.nvim_buf_add_highlight(buffer, -1, "GitTreeCommitBranch", k, commit_branch_from, commit_branch_to)
        end
    end
